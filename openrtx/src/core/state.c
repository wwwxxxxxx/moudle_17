...
void state_task()
{
     ...
     */
    uint16_t vbat = platform_getVbat();              /*m17为0*/
    state.v_bat  -= (state.v_bat * 2) / 100;         /*这样操作这有助于平滑电压的变化，以减少噪音和抖动。*/
    state.v_bat  += (vbat * 2) / 100;

    state.charge = battery_getCharge(state.v_bat);   /*获取电池充电状态和接收信号强度（RSSI）。*/
    state.rssi = rtx_getRssi();

    #ifdef RTC_PRESENT
    state.time = platform_getCurrentTime();			/*如果有RTC获取他的时间戳*/
    #endif

    pthread_mutex_unlock(&state_mutex);

    ui_pushEvent(EVENT_STATUS, 0);                   /*传递事件状态，给任务执行队列*/
}
